...
public interface DiaryRepository extends JpaRepository<Diary, Long> {
    ...
    @Query("select d from Diary d where d.folder= :folder and d.deleted = false order by d.date, d.createdAt")
    List<Diary> findAllForDiaryListAsc(@Param("folder") Folder folder, Pageable pageable);

    @Query("select d from Diary d where d.folder= :folder and d.deleted = false order by d.date desc , d.createdAt")
    List<Diary> findAllForDiaryListDesc(@Param("folder") Folder folder, Pageable pageable);
