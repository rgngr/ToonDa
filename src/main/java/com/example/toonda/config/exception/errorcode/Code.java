package com.example.toonda.config.exception.errorcode;
...
public enum Code {
    ...
    GET_COMMENTS("댓글 리스트 불러오기 성공", HttpStatus.OK ),
    GET_RECOMMENTS("대댓글 리스트 불러오기 성공", HttpStatus.OK ),
    ...
    DELETE_FOLDER( "폴더 삭제 성공", HttpStatus.OK),
    DELETE_DIARY( "다이어리 삭제 성공", HttpStatus.OK),
    CREATE_ENTER("모임 입장 성공", HttpStatus.OK),
    GET_PROFILE( "프로필 불러오기 성공", HttpStatus.OK),
    UPDATE_PROFILE_CONTENTS( "프로필 내용 수정 성공", HttpStatus.OK),
    UPDATE_PROFILE_IMG( "프로필 이미지 변경 성공", HttpStatus.OK),
    GET_PROFILE_UPDATE_PAGE( "프로필 수정 페이지 불러오기 성공", HttpStatus.OK),
    DELETE_PROFILE_IMG( "프로필 이미지 삭제 성공", HttpStatus.OK),
    ...
    NO_ANY_FOLDER("해당 유저는 생성한 폴더가 없습니다.", HttpStatus.NOT_FOUND),
    ...
    INVALID_USER("작성/수정/삭제 권한이 없는 유저입니다.", HttpStatus.BAD_REQUEST),
    ...
    SIGNUP_SUCCESS("회원가입 성공", HttpStatus.OK),
    USER_SIGNUP_FAIL("회원가입 실패", HttpStatus.BAD_REQUEST),
    EMAIL_CHECK("이메일 중복 확인 성공", HttpStatus.OK),
    USERNAME_CHECK("유저네임 중복 확인 성공", HttpStatus.OK),
    LOGIN_SUCCESS("로그인 성공", HttpStatus.OK),
    USER_FOLLOW_SUCCESS("유저 팔로우 성공", HttpStatus.OK),
    LIKE_FOLDER("폴더 구독/취소 성공", HttpStatus.OK),
    LIKE_DIARY("다이어리 좋아요/취소 성공", HttpStatus.OK),
    LIKE_COMMENT("댓글 좋아요/취소 성공", HttpStatus.OK),
    LIKE_RECOMMENT("댓글 좋아요/취소 성공", HttpStatus.OK),
    BLOCK_OR_CANCEL("차단/취소 성공", HttpStatus.OK),
    GET_BLOCK_LIST("차단 리스트 불러오기 성공", HttpStatus.OK),
    ...
    NO_BLOCKED_USER("회원을 찾을 수 없습니다.",HttpStatus.BAD_REQUEST),
